...
class GeneratorGRU():
    ...
    def _build_gragh(self):
        ...
        gru = GRU(self.parameters.generator_H, return_state=True, name='GRU')
        out, next_h= gru(out, initial_state=h_in)  # (B, H)
    ...
    def predict(self, state, stateful=True):
        ...
        prob, next_h = self.parameters.sess.run(
            [self.prob, self.next_h],
            feed_dict)
    ...
    def update(self, state, action, reward,is_episode_end, h=None, stateful=True): #o.81 train generator acording the current state
        #Update weights by Policy Gradient.
        if h is None:
            h = self.h
        state = state[:, -1].reshape(-1, 1)
        reward = reward.reshape(-1)
        feed_dict = {    # prep parameters for gru
            self.state_in : state,
            self.h_in : h,
            self.action : to_categorical(action, self.parameters.word_count),
            self.reward : reward}
        _, loss, next_h = self.parameters.sess.run(  # train
            [self.minimize, self.loss, self.next_h],
            feed_dict) #for each state we get loss and hidden state
        if stateful:
            self.h = next_h
    ...
    def generate_samples(self, parameters):  #o.48 from pre-train D
        self.parameters.strT.insert(self.parameters.END, "Generating fake sentences \n")
        sentences=[]
...
def GeneratorPretrainingGRU(parameters): #o.36 from train.init
    ...
    out = GRU(parameters.generator_H, return_sequences=True, name='GRU')(out)  # (B, T, H)
    out = TimeDistributed(
